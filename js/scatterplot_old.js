(function ($){	
			...
			var methods = {
				init : function(options){					
    				...
    				{
    					var dt = r.circle((leftgutter*2)+(i*leftgutter), settings.height - ((bottomgutter+5)+(i*bottomgutter)), 3).attr({stroke: "none", fill: settings.palette[Math.floor(Math.random()* settings.palette.length)]});	
    					circles[i] = dt;
    				}
					return this
				},
				animate : function(options){
				    ...
				    {    	
				    	...
				    	dt.animate({cx: animateX, cy: animateY, r:settings.dataZ[i]*(gridHeight/20),fill:settings.dataColor[i]},2000,"bounce");				    	
				    }
